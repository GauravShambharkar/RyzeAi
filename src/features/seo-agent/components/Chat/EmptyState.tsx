...
export const EmptyState = ({ onPromptSelect }: Props) => (
  <div className="flex-1 flex flex-col items-center justify-center py-12 px-2">
    ...
    <p className="text-xs md:text-sm text-neutral-600 mt-2 text-center max-w-md leading-6 tracking-tight">
    ...
    <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-2 w-full max-w-xl">
      {examplePrompts.map((p) => (
        <button
          ...
          className="group flex items-center justify-between text-left rounded-2xl border border-black/5 bg-white hover:border-emerald-500/30 hover:shadow-[0_12px_32px_-12px_rgba(20,20,20,0.1)] hover:-translate-y-0.5 text-xs md:text-sm text-neutral-700 px-4 py-3 transition-all"
        >
          <span>{p}</span>
