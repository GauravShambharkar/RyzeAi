...
import { ArrowLeft, LogOut, Plug, MessageSquare } from "lucide-react";
import { useLogin } from "@/features/seo-agent/hooks/login.hook";

type Props = { userEmail: string };

export const DashboardHeader = ({ userEmail }: Props) => {
      ...
      <div className="max-w-[1400px] mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <h1 className="hidden sm:inline font-display text-sm md:text-base text-neutral-700 tracking-tighter truncate">
            <span className="italic text-emerald-700">Dashboard</span>
          </h1>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <span className="text-xs text-neutral-500 hidden lg:inline font-mono truncate max-w-[200px]">
            {userEmail}
          </span>
          <button
            type="button"
