// Thin wrapper around the OpenAI Chat Completions API. Keeps handlers free of
// network details and gives GET/POST a shared success/error shape.
...
const getApiKey = () => process.env.NEXT_GPT_API_KEY ?? "";
...
export const callChat = async (
  ...
  if (!apiKey) {
    return { ok: false, status: 500, error: "Missing NEXT_GPT_API_KEY" };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const friendly =
        res.status === 429
          ? "OpenAI quota exceeded — check billing and usage limits."
          : res.status === 401
            ? "OpenAI API key is invalid or expired."
            : `OpenAI API ${res.status}: ${detail.slice(0, 200)}`;
    ...
    if (typeof text !== "string") {
      return { ok: false, status: 502, error: "Unexpected OpenAI response shape" };
    }
    return { ok: true, text };
  } catch (err) {
    return {
